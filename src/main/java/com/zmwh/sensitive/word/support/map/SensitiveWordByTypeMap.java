...
public class SensitiveWordByTypeMap implements IWordByTypeMap {
     ...
     * @since 1.0.0
     * 参考资料：https://blog.csdn.net/chenssy/article/details/26961957
     */
    @Override
    @SuppressWarnings("unchecked")
    public synchronized void initWordMap(HashMap<Integer, Set<String>> map,int mapSize) {
        long startTime = System.currentTimeMillis();
        // 避免扩容带来的消耗
        Map newInnerWordMap = new HashMap(mapSize);
    ...
    @Override
    public IWordResult containsP(String string, final IWordContext context) {
        if (StringUtil.isEmpty(string)) {
            return null;
        }
        ...
        for (int i = 0; i < string.length(); i++) {
            ...
            if (wordLength > 0) {
                // 保存敏感词
                String sensitiveWord = string.substring(i, i + wordLength);
                // 添加去重
                return WordResult2.newInstance()
                        .type(checkResult.sensitiveType())
                        .word(sensitiveWord);
    ...
    private List<IWordResult> getSensitiveWords(final String text, final ValidModeEnum modeEnum,
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (wordLength > 0) {
                ...
                // 添加去重
                WordResult2 wordResult = WordResult2.newInstance()
                        .type(checkResult.sensitiveType())
                        .word(sensitiveWord);
